...
//use lianpkg::core::config::{self, CreateConfigTomlInput};
...
//
//    let config_path = cli.config.unwrap_or_else(|| path::default_config_toml_path());
//
//    let create_out = config::create_config_toml(CreateConfigTomlInput { path: config_path.clone(), content: None });
//    if !create_out.ok {
//        logger::error(&format!("Config init error: {}", create_out.error.unwrap_or_else(|| "unknown error".to_string())));
//        return;
//    }
//    if create_out.created {
//        logger::info(&format!("Created default config at {:?}", create_out.path));
//    }
//
//    let mut config = match config::load_resolved_config(config_path) {
//        Ok(c) => c,
//        Err(e) => {
