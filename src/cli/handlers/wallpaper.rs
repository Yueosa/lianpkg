...
/// 预览模式
fn run_preview(workshop_path: &PathBuf, verbose: bool, ids: Option<&Vec<String>>) -> Result<(), String> {
    ...
    let wallpapers: Vec<_> = match ids {
        Some(filter_ids) => {
            let filtered: Vec<_> = result.wallpapers.iter()
                .filter(|w| filter_ids.contains(&w.wallpaper_id))
                .collect();
            
            // 检查是否有未找到的 ID
            let not_found: Vec<&str> = filter_ids.iter()
    ...
    out::info(&format!(
        ...
        if ids.is_some() { format!(", showing {}", wallpapers.len()) } else { String::new() }
    ));
    println!();
